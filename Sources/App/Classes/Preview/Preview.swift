...
class Preview {
    ...
    lazy var currentTempDirFolder: String = DirectoryManager.share.tempDirectory / String.random(10)
    var synthesizedInfo: [String: String] = ["ProvisionInfo": "", "BundleShortVersionString":  "", "BundleVersion": ""]
    lazy var html: String = {
        let path = DirectoryManager.share.resourcesDirectory / "template.html"
        let html = try? String(contentsOfFile: path)
        return html ?? ""
    }()
    
    init?(url: URL, socketUUID: String) {
        do {
...
*/
    ...
    func readAppInfo() {
        ...
        // 如果没有图标，就使用默认图标
        if appIcon == nil {
            let path = DirectoryManager.share.resourcesDirectory / "defaultIcon.png"
            appIcon = try? Data(contentsOf: path.fileURL)
        }
        let base64 = appIcon?.base64EncodedString() ?? ""
/*
...
*/
    ...
    /// 图标，描述文件没有图标
    func imageForApp(iconName: String) -> Data? {
        if dataType == .Ipa {
            ...
            do {
                ...
                return try? Data(contentsOf: appIconFullURL)
            } catch {
    ...
    func formattedDevices(_ value: [String]) -> [String: String] {
        ...
        let sortArray = value.filter({ !$0.isEmpty }).sorted(by: <)
        var currentPrefix = ""
