...
class UnZip {
    ...
    private let currentFold:URL
    
    init(url:URL) {
        self.url = url
        self.currentFold = (DirectoryManager.share.tempDirectory / String.random(10)).fileURL
    }
    
    func  run() throws {
        defer{ clear() }
        // 解压Info.plist和描述文件
        var regexs = [Regex("Payload/.*?\\.app/Info\\.plist"), Regex("Payload/.*?\\.app/embedded\\.mobileprovision")]
        ...
        if let bundleExecutable: String = appPropertyList.value(key: "CFBundleExecutable") {
            regexs = [Regex("Payload/.*?\\.app/\(bundleExecutable)")]
        ...
        regexs = [Regex("Payload/.*?\\.app/\(iconName).*?")]
        // 解压icon，优先使用高倍图
